...
public:
    ...
    void updateEnvironment(Perception p){
        ...
        p.getLocation() ? isDirtyA = p.getIsDirty() : isDirtyB = p.getIsDirty();    // ternario
...
int main(int argc, char *argv[]){
    ...
    SimpleAgent s;  // cria objeto
    int num;

    num = atoi(argv[1]);

    Environment ambiente(false, false, false);                    // isDirtyA, isDirtyB, agentLocation (1: A, 0: B)
